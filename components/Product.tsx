...
const Product = ({product}:{product:any}) => {
  ...
  return (
    <div className={`mt-12 w-full ${grid&&`flex flex-col`} ${list&&`flex gap-x-5`}`}>
            ...
            <Image src={builder.image(product?.image).url()} alt={product.name} width={500} height={500} className='w-full h-auto object-contain inset-0 transform scale-100 transition-transform duration-300 hover:scale-110' priority/>
