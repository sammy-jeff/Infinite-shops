...
const ProductContent = ({prod}:{prod:any}) => {
  ...
  return (
    <div className='grid grid-cols-1 h-[100%] lg:grid-cols-2 lg:w-[80%] bg-white overflow-auto '>
        ...
        <div className='p-4 relative flex flex-col'>
          ...
          <h3 className='mt-2 text-[#3a3a3a] font-semibold'>{formatPrice(prod?.price)}</h3>
          <p className='text-[#3a3a3a] mt-2 text-[clamp(13.6px,4vw,16px)]'>{prod?.description}</p>
          </div>
          ...
          <div className='grid grid-cols-3 w-[121.45px] h-[38px] border'>
                <button className='flex h-full justify-center items-center  border-r text-[#3a3a3a] text-[13px]' onClick={()=>setNum((prev:number)=>prev<=1?1:prev-1)}>-</button>
                <span className='h-full flex justify-center items-center  border-r text-[#3a3a3a] text-[13px]'>{num}</span>
                <button className='h-full flex justify-center mt-auto items-center text-[#3a3a3a] text-[13px]' onClick={()=>setNum((prev:number)=>prev+1)}>+</button>
              </div>
